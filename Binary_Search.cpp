...
Node binarySearch(vector<Node> &A, string target){
	int n = A.size();
	...
	while(low <= high){
		...
		if(A[mid].invN == target){
			return A[mid];
		}
		else if(A[mid].invN < target){
			low = mid + 1;
		}
		else{
			high = mid - 1;
...
int main() {
	...
	//reading a file
	ifstream csv_file("A2.1_ecommerce - A2.1_ecommerce - 2.csv");

	fout.open("A2.commerce2.1.c.csv", ios::out);
	...
	if (!csv_file) {
		...
		cout << "Type the invoice number you want to search :" << '\n';
		string target;
		cin >> target;
		Node ans = binarySearch(data, target);

		if(ans.desT == "null"){
			cout << "Item Not Found" << '\n';
		}
		else{
		    cout << ans.invN << ", " << ans.stcode << ", " << ans.desT << ", " << ans.quanT << ", " << ans.invD << ", " << ans.unitP << ", " << ans.cusId << ", " << ans.cntry <<'\n';
	    }
		cout << '\n';
